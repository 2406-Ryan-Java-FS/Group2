...
public interface AuctionRepo extends JpaRepository<Auction, Integer> {
    @Modifying
    @Transactional
    @Query("update Auction a set a.bid = :bid, a.bidder_id = :bidder_id where a.a_id = :id")
    int updateAuctionBidById(@Param("id") int id, @Param("bid") double bid, @Param("bidder_id") int bidder_id);

    @Modifying
    @Transactional
    @Query("update Auction a set a.time = :status where a.a_id = :id")
    int updateAuctionStatusById(@Param("id") int id, @Param("status") String status);

    @Modifying
    @Transactional
    @Query("update Auction a set a.time = :time where a.a_id = :id")
